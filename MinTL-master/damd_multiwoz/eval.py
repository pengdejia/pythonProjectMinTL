...
class MultiWozEvaluator(object):
    ...
    def _constraint_compare(self, truth_cons, gen_cons, slot_appear_num=None, slot_correct_num=None, false_slot_dict=None):
        tp,fp,fn = 0,0,0
        false_slot = []
        for slot in gen_cons:
            ...
            if slot in truth_cons and self.value_similar(v_gen, truth_cons[slot]):  #v_truth = truth_cons[slot]
                ...
                fp += 1
                if false_slot_dict is not None:
                    false_slot_dict[slot] = 1 if not false_slot_dict.get(slot) else false_slot_dict.get(slot) + 1
    ...
    def dialog_state_tracking_eval(self, data, eval_dial_list = None, bspn_mode='bspn', no_name=False, no_book=False, multi_label_true_list=None):
        dials = self.pack_dial(data)
        total_turn, joint_match, total_tp, total_fp, total_fn, total_acc = 0, 0, 0, 0, 0, 0
        slot_appear_num, slot_correct_num, false_slot_dict = {}, {}, {}
        dial_num = 0
        false_slots_list = []
        error_value_count = 0
        all_right_count = 0
        ...
        for dial_id in dials:
            ...
            for turn_num,turn in enumerate(dial):
                ...
                if truth_cons.keys() == gen_cons.keys():
                    domain_slot_count += 1

                if truth_cons == gen_cons:
                    joint_match += 1
                    if multi_label_true_list is not None:
                        if multi_label_true_list[total_turn] == 1:
                            all_right_count += 1
                            slot_right_count += 1
                else:
                    if truth_cons.keys() == gen_cons.keys():
                ...
                if eval_dial_list is None:
                    ...
                    tp,fp,fn, acc, false_slots = self._constraint_compare(truth_cons, gen_cons,)
                false_slots_list.extend(false_slots)
                total_tp += tp
                total_fp += fp
                total_fn += fn
                total_acc += acc
        ...
        all_right_count = all_right_count / (total_turn + 1e-10) * 100
        print(all_right_count)
        slot_right_count = slot_right_count / (total_turn + 1e-10) * 100
        print(slot_right_count)
        # print(slot_appear_num)
        # print(slot_correct_num)
        # print(list(set(false_slots_list)))
        # print(error_value_count)
        print("domain slot accuracy {}".format(domain_slot_count/(total_turn+1e-10) * 100))
        # print(false_slot_dict)
        return joint_goal, f1, accuracy, slot_appear_num, slot_correct_num, total_turn
